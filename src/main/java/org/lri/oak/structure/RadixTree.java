 ...
 * 
 * @authors Tahseen Ur Rehman, Paul Lagree
...
public interface RadixTree {
	 ...
	 */
	public void insert(String key, float value) throws DuplicateKeyException; // OK
	 ...
	 * @return
	 */
	public boolean delete(String key); // OK

	/**
	 * Find a value based on its corresponding key.
	 * 
	 * @param key The key for which to search the tree. Find only exact matches (if key=p, search for word p 
	 * and IF p does not exist, search for prefix p of words p+suffix.
	 ...
	 * 
	 * @param prefix The prefix for which keys need to be search
	 * @param exact If the prefix has a match, returns the node corresponding to it, even if words starting
	 * with that prefix have greater values
	 * @return The node which has this prefix
	 */
	public RadixTreeNode searchPrefix(String prefix, boolean exact); // OK
